...
class Fraction(QtWidgets.QMainWindow):
	...
	def __init__(self,data_frame, mida_p, df_closest, f_transfer, parent = None):
		...
		self.mida_p = mida_p
		self.df_closest = df_closest
		self.fractional_abundances = f_transfer
		...
		if np.isnan(self.mida_p):
			QtWidgets.QMessageBox.information(self,"Error", "First calculate MIDA p")
	...
	def calc_f(self):
		...
		print(index)
		abundance = self.fractional_abundances[index]

		em_col_name = 'EM'+str(index)
		nat_col_name = 'M'+str(index)


		nat_abd = self.data.iloc[0,index+1]
		measured_enrichment = abundance - nat_abd
		print(abundance)
		print(nat_abd)
		asymptotic_enrichment = float(self.df_closest[em_col_name])
		f = float(measured_enrichment/asymptotic_enrichment)
...
class MainWindow(QtWidgets.QMainWindow):
	...
	def __init__(self,parent = None):
		...
		uic.loadUi(os.path.join(location,'main_ui_draft.ui'),self)
		...
		#######
		self.m0_input = self.findChild(QLineEdit, 'M0')
		self.m1_input = self.findChild(QLineEdit, 'M1')
		self.m2_input = self.findChild(QLineEdit, 'M2')
		self.m3_input = self.findChild(QLineEdit, 'M3')
		self.m4_input = self.findChild(QLineEdit, 'M4')
		#######

		self.mida_p = np.nan

		calc_p_push = self.findChild(QPushButton, 'calc_p')
		calc_p_push.clicked.connect(self.calculate_p)
	...
	def start_calc_f(self):
		...
		self.f_window = Fraction(df, self.mida_p, self.df_closest, self.f_transfer)
		self.f_window.show()
		self.close()
	...
	def create_table(self):
		...
		df.insert(loc = 0, column = 'p', value = increments) #change column name to N
	...
	def create_csv(self):
		...
		# df.insert(loc = 0, column = 'p', value = increments) #CHANGE TO WRITE N INSTEAD
		...
		#FIX WITH ENRICHMENT COLUMN NAMES

	def calculate_p(self):

		#edge case, does not fall within range. error message:
		#check if qlineedit is empty

		if self.m0_input.text() == "" or self.m1_input.text() == "" or self.m2_input.text() == "":
			QtWidgets.QMessageBox.information(self,"Error", "Isotopomer envelope must include at minimum M0-M2")

		# if self.m4_input.text() == "":
		# 	self.m4_input = 0
		# if self.m3_input.text() == "":
		# 	self.m3_input = 0
		...
		for index,row in df.iterrows():
			if index == 0: continue
			ratio_array.append(row['EM2']/row['EM1'])
		
		df['ratio'] = ratio_array


		fractional_abundances = [self.m0_input.text(), self.m1_input.text(), self.m2_input.text()]
		if self.m3_input.text(): 
			fractional_abundances.append(self.m3_input.text())
		if self.m4_input.text(): 
			fractional_abundances.append(self.m4_input.text())

		
	

		#fractional_abundances = [self.m0_input.text(), self.m1_input.text(), self.m2_input.text(), self.m3_input.text(), self.m4_input.text()]
		fractional_abundances = [float(x) for x in fractional_abundances]
		self.f_transfer = fractional_abundances
		print(self.get_num_isotopomers())
		print(len(fractional_abundances))
		if self.get_num_isotopomers() != len(fractional_abundances):
			QtWidgets.QMessageBox.information(self,"Error", "Number of isotopomers specified must match inputted data")
		if (sum(fractional_abundances) > 1.01) or (sum(fractional_abundances) < 0.99):
			QtWidgets.QMessageBox.information(self,"Error", "Fractional abundances must sum to unity")

		nat_obj = Abundance(formula)
		np_array = nat_obj.get_MID()[0]

		nat_abd_array = np_array[:self.get_num_isotopomers()]
		nat_abd_array = normalization(nat_abd_array)

		enrichments = fractional_abundances - nat_abd_array
		em2em1_ratio = enrichments[2]/enrichments[1]
		


		self.df_closest = df.iloc[(df['ratio'].astype(float) - em2em1_ratio).abs().argsort()[:1]]


		self.mida_p = float(self.df_closest['p'])
		print(self.mida_p)
		msg = QMessageBox()
		msg.setWindowTitle("MIDA p")
		msg.setText(str(self.mida_p))
		msg.exec_() 

		# enrich_objt = Abundance(self.get_formula, self.get_num_labile, p=self.enrichment, heavy_element = self.get_label)
		# nat_obj = Abundance(self.get_formula)

		# enrichments = enrich_obj.get_MID[:5] - nat_obj.get_MID[:5]
		# em2em1_ratio = enrichments[3]/enrichments[2]
		# print(em2/em2em1_ratio)
